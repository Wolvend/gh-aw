 ...
 * that checkout repositories into tmp/gh-aw/repo-imports/ (relative to GITHUB_WORKSPACE), and mergeRepositoryGithubFolder uses those.
...
async function mergeRepositoryGithubFolder(owner, repo, ref, workspace) {
  ...
  const relativePath = `tmp/gh-aw/repo-imports/${owner}-${repo}-${sanitizedRef}`;
